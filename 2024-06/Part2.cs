...
public static class Part2 {
  ...
  private static Dictionary<Complex, HashSet<Direction>> visited = new();
  ...
  public static bool PatrolAndTellLoop() {
    if (visited.ContainsKey(position)) {
      bool hasBeenAdded = visited[position].Add(direction);
      if (!hasBeenAdded) {
        return true; // must be a loop
      }
    } else {

      visited[position] = new HashSet<Direction>();
  ...
  public static string Solve(List<String> input) {
    ...
    HashSet<Complex> possibleObstaclePlaces = new(visited.Keys);

    int result = 0;
    foreach (Complex pos in possibleObstaclePlaces) {
      ...
      bool looping = false;
      while (InBounds() && !looping) {
        looping = PatrolAndTellLoop();
      }
